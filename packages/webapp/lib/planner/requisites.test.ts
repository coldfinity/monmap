...
test("FIT2004 shape: need one from each OR group", () => {
  ...
  assert.deepEqual(
    r.missingCodes,
    ["FIT1058", "FIT2085", "FIT1054", "MAT1830"].sort()
  )
...
test("missing connector defaults to AND", () => {
  ...
  assert.equal(
    evaluateRequisiteTree(rule, new Set(["FIT1008"])).satisfied,
    false
  )
  assert.equal(
...
test("prohibition: satisfied when none of the referenced codes are taken", () => {
  ...
  assert.deepEqual(evaluateProhibition(rule, new Set(["FIT1045", "FIT1053"])), {
    satisfied: false,
    conflictingCodes: ["FIT1045", "FIT1053"],
  })
})
